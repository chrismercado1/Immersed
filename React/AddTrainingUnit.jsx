...
function AddTrainingUnit() {
  ...
  useEffect(() => {
    if (state?.type === "Edit_TrainingUnit") {
      ...
      const trainingUnitEdits = state.payload.id;

      setUserFormData((prevState) => {
        const newTrainingUnitData = { ...prevState, ...state.payload };
        newTrainingUnitData.payload = trainingUnitEdits;
  ...
  const onGetOrgSuccess = (response) => {
    let allOrgsByCurrentUser = response.items;
    _logger("allOrgsByCurrentUser is firing", allOrgsByCurrentUser);

    setFormInfo((prevState) => {
      let mappedInfo = { ...prevState };
      mappedInfo.orgCurrentUser = allOrgsByCurrentUser.map(mapOrgCurrentUser);
      return mappedInfo;
    });
  };
  ...
  const onGetOrgError = (err) => {
    _logger("error", err);
    toastr.error("Failed to select an Organization");
  };
  ...
  const mapOrgCurrentUser = (aOrgUser) => {
    return (
      <option key={`List-B` + aOrgUser.id} value={aOrgUser.id}>
        {aOrgUser.name}
      </option>
    );
  };
  ...
  const onAddSuccess = (response) => {
    setUserFormData((prevState) => {
      const addNewTrainingUnit = { ...prevState };

      addNewTrainingUnit.id = response.item;
      return addNewTrainingUnit;
    });
